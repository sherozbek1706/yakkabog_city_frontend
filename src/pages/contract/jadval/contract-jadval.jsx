...
const ContractJadval = () => {
  ...
  // Function to group data into vertical columns
  const groupedMonthlyData = () => {
    const allMonthsData = getMonthlyDatesWithAmounts(
      shartnoma?.created_at,
      shartnoma?.muddatli_tulov_oyi,
    ...
    const half = Math.ceil(allMonthsData.length / 2);
    const firstColumn = allMonthsData.slice(0, half);
    const secondColumn = allMonthsData.slice(half);
        ...
        <div className="ContractPDF__page">
          <div>
            ...
            <table className="Contract__table">
              ...
              <tbody>
                {(() => {
                  const [firstColumn, secondColumn] = groupedMonthlyData();
                  ...
                  return Array.from({ length: maxLength }, (_, index) => (
                    <tr key={index}>
                      {[firstColumn[index], secondColumn[index]].map(
                        (elem, colIndex) =>
                          elem ? (
                            <Fragment key={colIndex}>
                              <td>
                                {index + 1 + colIndex * firstColumn.length}
                              </td>
                              <td>
                                {getDayFromDate(elem?.date)}-
                                {getMonthName(elem?.date)}{" "}
                                {getYearFromDate(elem?.date)}-yil
                              </td>
                              <td>{formatmoney(elem?.amount)} so'm</td>
                            </Fragment>
                          ) : (
